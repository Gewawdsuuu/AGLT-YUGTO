...
public class ThunderboltAbility : MonoBehaviour
{
    ...
    public void ThunderSpawn()
    {
        GameObject newThunderObject;

        for (int i = 0; i < enemyGameObjects.Count; i++)
        {
            newThunderObject = Instantiate(ability.abilityPrefab, new Vector3(enemyGameObjects[i].transform.position.x, enemyGameObjects[i].transform.position.y, enemyGameObjects[i].transform.position.z), transform.rotation);
        }
    }
}
